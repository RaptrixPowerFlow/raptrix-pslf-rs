...
**Canonical local RPF corpus:** `tests/golden/` (regenerated by `cargo test --test golden_test`). Prefer that path over scratch `tests/compare/` outputs, which can go stale.

**Coverage note:** Eastern Interconnect / Midwest24k exist only as PSS/E RAW under `raptrix-psse-rs` — there are no PSLF EPC decks in this repo. Those cases are owned by the psse-rs golden corpus.
...
- `scripts/generate_all_rpfs.sh` — regenerate `tests/golden/*.rpf` from local `tests/networks/` (DYD attached when present)
...
Golden policy (parity with `raptrix-psse-rs`): canonical `tests/golden/<stem>.rpf` includes the matching `.dyd` when available; `_dynamic` / `_static` twins are also written.
