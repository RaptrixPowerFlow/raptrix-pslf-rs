...
- Emits the full set of **18** canonical required RPF **v0.13.0** root tables (zero-row where appropriate). Nullable `buses.latitude` / `buses.longitude` are always null (no WGS84 in EPC).
...
This crate is pinned to **`raptrix-cim-arrow` 0.6.0** (git tag `v0.6.0`), matching the current `raptrix-psse-rs` release. Every emitted `.rpf` is validated against the locked **v0.13.0** contract before returning and carries synthesized **`mrid`** identifiers on equipment tables. Pre-0.13 files must be re-exported; see [MIGRATION.md](MIGRATION.md).
